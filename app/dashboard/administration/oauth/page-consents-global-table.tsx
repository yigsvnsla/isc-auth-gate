...
import { useListOAuthConsentsGlobalQuery } from "./list-oauth-consents-global-query";
import { GlobalConsentsTableColumns } from "./page-consents-global-columns";
import {
  flexRender,
  getCoreRowModel,
  useReactTable,
...
export const GlobalAuthConsentsTable: FC = () => {
  ...
  const { data, isLoading, mutate } = useListOAuthConsentsGlobalQuery();
  ...
  const filteredData = useMemo(() => {
    ...
    return rawData.filter(
      (consent) =>
        ...
        (consent.userEmail ?? "").toLowerCase().includes(searchLower) ||
        (consent.userName ?? "").toLowerCase().includes(searchLower),
    );
  }, [rawData, search]);
  ...
  const table = useReactTable({
    columns: GlobalConsentsTableColumns,
    data: paginatedData,
      ...
      <>
        ...
        <div className="overflow-hidden rounded-md border">
          <Table>
            <TableHeader>
              <TableRow>
                {GlobalConsentsTableColumns.map((_, i) => (
                  <TableHead key={i}>
  ...
  return (
    <>
      ...
      <div className="relative flex-1">
        <Input
          placeholder="Search consents by client, user, or ID..."
          value={search}
          ...
          <TableBody>
            {!hasData ? (
              <TableRow className="hover:bg-transparent">
                <TableCell colSpan={GlobalConsentsTableColumns.length}>
                  <Empty className="h-[calc(10*52px)]">
                    <EmptyHeader>
                      ...
                      <EmptyDescription className="max-w-xs text-pretty">
                        Consents appear after a user authorizes a third-party
